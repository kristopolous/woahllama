...
def main():
    if DB.exists():
        # A full re-ingest is the only way to pick up new commits, but it takes
        # server_geo with it and geo.py cannot rebuild that without the db-ip
        # CSV, which is not in the repo. Save the placements first; rebuild.sh
        # restores them by URL after the merges. Everything else in survey.db is
        # regenerated by the pipeline.
        import subprocess
        subprocess.run([sys.executable, str(pathlib.Path(__file__).parent / "geo_keep.py"),
                        "save"], check=False)
        DB.unlink()
